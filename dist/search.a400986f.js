...
async function fetchData(query) {
    ...
    try {
        const response = await _axiosDefault.default.get(URI + query);
        ...
        if (response.data[0].currencies.length === 1) thirdLine.textContent = `The capital is ${capital} and you pay with ${response.data[0].currencies[0].name}`;
        else thirdLine.textContent = `The capital is ${capital} and you pay with ${response.data[0].currencies[0].name} or with ${response.data[0].currencies[1].name}`;
...
function createFourthLine(languages) {
    ...
    if (languages.length > 1) for(let i = 1; i < languages.length; i++)lineString = lineString + " and " + languages[i].name;
...
},{"./../utils.js":"60BxC","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["1fSmN","hebny"], "hebny", "parcelRequire5e27")
